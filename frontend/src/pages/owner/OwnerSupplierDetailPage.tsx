    ...
    >
      ...
      {tab === 'الخدمات' || tab === 'التسعير' ? (
        <div className="grid gap-4">
          ...
          <ul className="space-y-2">{services.map((item) => <li key={String(item.id)} className="rounded-xl border bg-white p-3 text-sm">{String(item.name)} · {String(item.pricing_model)} · {String(item.minimum_price ?? '—')}–{String(item.maximum_price ?? '—')} {String(item.currency ?? '')}</li>)}</ul>
        </div>
      ) : null}
      ...
      {tab === 'المعرض' ? (
        <ul className="space-y-2">{portfolio.map((item) => <li key={String(item.id)} className="rounded-xl border bg-white p-3">{String(item.title)} · {String(item.status)} · {String(item.visibility ?? '—')}</li>)}</ul>
      ) : null}
      {tab === 'المنتجات' ? (
        <ul className="space-y-2">{products.map((item) => <li key={String(item.id)} className="rounded-xl border bg-white p-3">{String(item.name)} · {String(item.status)} · {String(item.visibility ?? '—')}</li>)}</ul>
      ) : null}
      {tab === 'السجل' ? (
